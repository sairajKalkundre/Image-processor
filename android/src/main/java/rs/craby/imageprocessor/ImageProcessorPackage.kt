package rs.craby.imageprocessor
...
class ImageProcessorPackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyImageProcessor_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-image-processor")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return ImageProcessorPackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
